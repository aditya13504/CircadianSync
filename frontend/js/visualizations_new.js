...
class Visualizations {
    ...
    // Update color temperature bar
    updateColorTempBar(colorTemp) {
        const bar = document.getElementById('temp-visualization');
        if (!bar) return;
        
        // Calculate position on temperature scale (2000K - 10000K)
        const minTemp = 2000;
        const maxTemp = 10000;
        const position = Math.max(0, Math.min(100, ((colorTemp - minTemp) / (maxTemp - minTemp)) * 100));
        
        bar.style.background = `linear-gradient(to right, 
            #ff4500 0%, #ff8c00 25%, #ffff00 50%, #87ceeb 75%, #4169e1 100%)`;
        bar.style.position = 'relative';
        
        // Add temperature indicator
        bar.innerHTML = `<div style="position: absolute; left: ${position}%; top: 50%; 
            width: 4px; height: 100%; background: #fff; transform: translateX(-50%);"></div>`;
    }
